...
def fit_GMMs(mdl, path, use_gmms, Kcomps, all_hhats, dontfit=False):
    '''
    This function fits a GMM on the embeddings given by 
    all_hhats, and then assigns the learned GMM to the model represented by mdl. 
    '''
    if use_gmms[0]:
        if dontfit and os.path.exists(path + 'Kmog{}.gmm'.format(Kcomps)):
            pickle.load(open(path + 'Kmog{}.gmm'.format(Kcomps), 'rb'))
        else:
            ...
            pickle.dump(GMM, open(path + 'Kmog{}.gmm'.format(Kcomps), 'wb'))
            mdl.initialize_GMMparams(GMM)

    if use_gmms[1]:
        if dontfit and os.path.exists(path + 'Kmog{}.bgmm'.format(Kcomps)):
            pickle.load(open(path + 'Kmog{}.bgmm'.format(Kcomps), 'rb'))
        else: 
            ...
            pickle.dump(BGMM, open(path + 'Kmog{}.bgmm'.format(Kcomps), 'wb'))
            mdl.GMM = BGMM
...
argparser.add_argument('--data', type=str, help='dataset, currently mnist/celeba', default='celeba')
...
if model == 'NF':
    ...
    for config_num, Ks in enumerate(Kss):
        ...
        # do the additional iterations for VAE training
        path_add = 'models/VAEaddit_{}_K_{}.t'.format(arguments.data, Ks)
        if 1 & os.path.exists(path_add):
            mdl.load_state_dict(torch.load(path_add))
        else:
            mdl.VAE_trainer(arguments.cuda, train_loader, vis=vis, 
                            EP=EP, config_num=config_num)
            torch.save(mdl.state_dict(), path_add)

        all_hhats_add = get_embeddings(mdl, train_loader)
        all_hhats_add_chunks = torch.chunk(all_hhats, chunks=35, dim=0)

        if compute_vae_scores:
            scores_vae, samples_vae = get_scores(test_loader, mdl, arguments.cuda, 
                                       num_samples=num_samples,
                                       task='celeba', base_dist='fixed_iso_gauss')
            pickle.dump(scores_vae, 
                        open('/home/cem/Dropbox/GANs/UAI2018/mtl_ai_pres/vaeadd_celeba.pk', 'wb'))
            torchvision.utils.save_image(0.5*samples_vae[:64] + 0.5,  '/home/cem/Dropbox/GANs/UAI2018/mtl_ai_pres/samples_vaeadd_celeba.png')

        scores_iml_all = []
        for Kcomps in range(10, 51, 5):
            print('Number of GMM components {}'.format(Kcomps))
            path2 = 'models/VaDEGMM_{}_K_{}_Kmog_{}.t'.format(arguments.data, Ks, Kcomps)

            # initialize and load the model parameters
            mdl = conv_VAE(L, L, Ks, M=64, num_gpus=arguments.num_gpus) 
            mdl_vade = conv_VAE(L, L, Ks, M=64, num_gpus=arguments.num_gpus) 

            if arguments.cuda:
                mdl.cuda()
                mdl_vade.cuda()
            mdl.load_state_dict(torch.load(path_add)) # we are loading the VAE with additional iterations
            mdl_vade.load_state_dict(torch.load(path)) # we are loading the regular VAE here

                       
            if compute_iml_scores:
                # this is for IML
                fit_GMMs(mdl, path_add, use_gmms, Kcomps, all_hhats_add, dontfit=True) 
            ...
            if run_vade_model:
                # this is for VaDE
                fit_GMMs(mdl_vade, path2, [1, 0, 0], Kcomps, all_hhats, dontfit=False) 
                if arguments.cuda:
                    mdl_vade.cuda()

                if 0 & os.path.exists(path2):
                    mdl_vade.load_state_dict(torch.load(path2))
                else:
                    mdl_vade.VAE_trainer_mog(arguments.cuda, train_loader, vis=vis, 
                                        EP=EP, config_num=config_num, data='celeba')

                    torch.save(mdl_vade.state_dict(), path2)

                scores_vade, gen_data_vade = get_scores(test_loader, mdl_vade, arguments.cuda, 
                                                        num_samples=num_samples,
                                                        task='celeba', base_dist='mog')
                ...
                pickle.dump(results, open('/home/cem/Dropbox/GANs/UAI2018/mtl_ai_pres/vade_celeba_{}.pk'.format(Kcomps), 'wb'))
